...
<%
	...
	*/
	//DataSource source=(DataSource)ctx.lookup("jdbc/jsp");
	//풀 경로로 접근하기
	  DataSource source=(DataSource)ctx.lookup(application.getInitParameter("TOMCAT_JNDI_ROOT")+"/jdbc/jsp");
	  /* 
	   4]커넥션pool에서 톰캣 서버가 생성해 놓은 
	    Connection객체를 갖다 쓰자
	    DataSource의 getConnection()메소드로 
	    톰캣이 pool에 미리 생성해 놓은  
	    Connection객체를 가져다 쓴다.
	   */ 
	 Connection conn = source.getConnection();
	   String connString;
	   if(conn !=null) connString="<h2>데이타베이스 연결 성공</h2>";
	   else connString="<h2>데이타베이스 연결 실패</h2>";		   
